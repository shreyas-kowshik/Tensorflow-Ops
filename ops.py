...
def get_W(shape,initializer=tf.contrib.layers.xavier_initializer(),name='W'):
...
def D_conv2d(x,ksize,out_ch,stride,padding='SAME',name='D_conv2d'):
    ...
    if padding == 'SAME':
        output_shape = [shape[0],shape[1]*stride,shape[2]*stride,out_ch]
    else:
        output_shape = [shape[0],(shape[1] - 1)*stride + ksize,(shape[2] - 1)*stride + ksize,out_ch]
...
def pad(x,padding,mode='CONSTANT'):
    '''
    padding : non-square padding values for each axis
              example : [[0,0],[2,2],[2,2],[0,0]]
    '''
...
def bn(x,is_train,name='bn'):
    ...
    '''
    with tf.variable_scope(name):
        return tf.contrib.layers.batch_norm(x,is_training=is_train,scale=True,center=True)
...
def test():
	...
	bn1 = bn(X,True)
	print(bn1.shape)
	c1 = pad(bn1,[[0,0],[1,1],[1,1],[0,0]])
	c1 = conv2d(c1,4,16,2,name='c1',padding='VALID')
	print(c1.shape)
	a1 = lrelu(c1,name='a1')
	print(a1.shape)
	p1 = max_pool(a1,2,2)
